...
/* =========================================================
   Page
========================================================= */
export default function Paket14RentACarClient() {
  /* ---- Search state ---- */
    ...
    <main className="min-h-[100dvh] bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto max-w-7xl px-4 py-14 md:py-24 grid md:grid-cols-12 gap-10 items-center">
          ...
          <div className="md:col-span-6">
            ...
            <h1 className="mt-4 text-4xl md:text-6xl font-extrabold leading-[1.06]">
              ...
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-amber-300 via-yellow-2  00 to-amber-400 drop-shadow-[0_0_32px_rgba(245,158,11,0.35)]">
                Bir Adım Önde
              </span>
            </h1>
      ...
      <section id="cars" className="mx-auto max-w-7xl px-4 py-14 border-t border-white/10">
        ...
        <div className="mt-4 grid lg:grid-cols-5 gap-3">
          ...
          >
            {['All', 'Economy', 'Compact', 'SUV', 'Premium', 'Luxury', 'Van'].map((s) => (
        ...
        <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((c) => {
            ...
            return (
              <article key={c.id} className="group rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
                <div className="relative">
                  ...
                  {c.popular && (
                    <span className="absolute right-3 top-3 text-[11px] px-2 py-1 rounded-full bg-amber-400 text-slate-9 00 font-semibold">
                      Popüler
                    </span>
                  )}
                </div>
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xl font-extrabold">{formatPrice(c.pricePerDay)}</div>
                      <div className="mt-1 text-xs text-slate-400">
                        {duration ? `Toplam: ${formatPrice(basePrice(c))}` : 'Tarih seçin'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={
                        ...
                        (compare.includes(c.id) ? 'border-amber-400 text-amber-300' : 'border-white/15')
                      }
                    >
      ...
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 h-16 flex items-center justify-between text-sm text-slate-400">
          ...
          <a href="#search" className="underline underline-offset-4">Rezervasyon</a>
        </div>
      </footer>
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={'h-6 w-11 rounded-full p-0.5 flex items-center ' + (checked ? 'bg-emerald-500' : 'bg-slate-600')}
      aria-label="toggle"
    >
  ...
  return (
    <button
      ...
      className={
        ...
        (active ? 'bg-amber-400 text-slate-900 border-amber-300' : 'bg-slate-900/60 text-slate-200 border-white/10 hover:bg-white/5')
      }
    >
      {children}
    </button>
  ...
  return (
    <div className="flex items-center">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} viewBox="0 0 20 20" className={'h-4 w-4 ' + (i < s ? 'fill-amber-400' : 'fill-slate-400/40')}>
...
function CompareDrawer({
  ...
  return (
    <div className={'fixed inset-y-0 right-0 z-50 w-[92%] sm:w-[720px] bg-slate-950/95 backdrop-blur border-l border-white/10 transition-transform ' + (open ? 'translate-x-0' : 'translate-x-full')}>
      ...
      <div className="relative h-full flex flex-col">
        <div className="h-14 px-4 border-b border-white/10 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-lg border border-white/10 px-3 py-1.5 text-sm hover:bg-white/5">Kapat</button>
        </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {list.map((c) => (
                <div key={c.id} className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <Image src={c.image} alt={`${c.brand} ${c.model}`} width={320} height={180} className="h-32 w-full object-cover rounded-lg" />
                  <div className="mt-3 font-semibold">{c.brand} {c.model}</div>
                  <div className="mt-1 text-xs text-slate-300">
                    {c.segment} · {c.transmission === 'Automatic' ? 'Otomatik' : 'Manuel'} · {fuelLabel(c.fuel)}
                  </div>
                  <div className="mt-1 text-xs text-slate-400">🚪 {c.doors} · 🧳 {c.luggage} · 🪑 {c.seats}</div>
                  <div className="mt-3 text-sm">Günlük: <span className="font-semibold">{formatPrice(c.pricePerDay)}</span></div>
                  <div className="text-xs text-slate-400">{durationDays} gün toplam: {formatPrice(c.pricePerDay * durationDays)}</div>
                </div>
              ))}
            </div>
          )}
...
function ExtrasDrawer({
  ...
  return (
    <div className={'fixed inset-y-0 right-0 z-50 w-[92%] sm:w-[480px] bg-slate-950/95 backdrop-blur border-l border-white/10 transition-transform ' + (open ? 'translate-x-0' : 'translate-x-full')}>
      <div className={'fixed inset-0 bg-black/40 transition ' + (open ? 'opacity-100' : 'opacity-0 pointer-events-none')} onClick={onClose} />
      <div className="relative h-full flex flex-col">
        <div className="h-14 px-4 border-b border-white/10 flex items-center justify-between">
        ...
        <div className="p-4 space-y-4 overflow-auto">
          {!car ? (
            <div className="rounded-xl border border-white/10 p-6 text-center text-sm text-slate-300">Araç seçiniz.</div>
          ) : (
            <>
              <div className="rounded-xl border border-white/10 p-4 flex items-center gap-3">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-slate-300">{durationDays} gün · {formatPrice(car.pricePerDay)} /gün</div>
                </div>
              </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <label className="flex items-start gap-3">
                  <input
                    ...
                    onChange={(e) => setExtras({ ...extras, fullProtection: e.target.checked })}
                  />
                  <div className="flex-1">
                    <div className="font-medium">
                      {EXTRAS_PRICING.fullProtection.label}{' '}
                      <span className="text-slate-300">· {formatPrice(EXTRAS_PRICING.fullProtection.perDay!)} /gün</span>
                    </div>
                ...
                <label className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="font-medium">
                      ...
                      <span className="text-slate-300">· {formatPrice(EXTRAS_PRICING.additionalDriver.perDay!)} /gün</span>
                    </div>
                  </div>
                </label>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-slate-300 text-xs">{formatPrice(EXTRAS_PRICING.childSeat.perDay!)} /gün · Adet</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setExtras({ ...extras, childSeat: Math.max(0, extras.childSeat - 1) })} className="h-8 w-8 rounded-lg border border-white/10">−</button>
                    <span className="w-6 text-center">{extras.childSeat}</span>
                    <button onClick={() => setExtras({ ...extras, childSeat: Math.min(3, extras.childSeat + 1) })} className="h-8 w-8 rounded-lg border border-white/10">+</button>
                  </div>
                </div>
                ...
                <label className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) => setExtras({ ...extras, navigation: e.target.checked })}
                  />
                  <div className="flex-1">
                    <div className="font-medium">
                      {EXTRAS_PRICING.navigation.label}{' '}
                      <span className="text-slate-300">· {formatPrice(EXTRAS_PRICING.navigation.fixed!)} (tek sefer)</span>
                    </div>
                  </div>
                </label>
                ...
                <label className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) => setExtras({ ...extras, winterTires: e.target.checked })}
                  />
                  <div className="flex-1">
                    <div className="font-medium">
                      {EXTRAS_PRICING.winterTires.label}{' '}
                      <span className="text-slate-300">· {formatPrice(EXTRAS_PRICING.winterTires.perDay!)} /gün</span>
                    </div>
                  </div>
                </label>
              </div>
              ...
              <div className="rounded-xl border border-white/10 p-4">
                ...
                <div className="mt-1 text-xs text-slate-400">Günlük ücretli kalemler {durationDays} gün ile çarpılır.</div>
              </div>

              <button onClick={onContinue} className="w-full h-11 rounded-xl bg-amber-400 text-slate-900 font-semibold hover:bg-amber-300">
...
function BookingDrawer({
  ...
  return (
    <div className={'fixed inset-y-0 right-0 z-50 w-[92%] sm:w-[520px] bg-slate-950/95 backdrop-blur border-l border-white/10 transition-transform ' + (open ? 'translate-x-0' : 'translate-x-full')}>
      <div className={'fixed inset-0 bg-black/40 transition ' + (open ? 'opacity-100' : 'opacity-0 pointer-events-none')} onClick={onClose} />
      <div className="relative h-full flex flex-col">
        <div className="h-14 px-4 border-b border-white/10 flex items-center justify-between">
        ...
        <div className="p-4 space-y-4 overflow-auto">
          {!car ? (
            ...
            <>
              ...
              <div className="rounded-xl border border-white/10 p-4">
                <div className="flex items-center gap-3">
                  <Image src={car.image} alt={car.model} width={96} height={64} className="object-cover rounded-lg" />
                  <div className="text-sm">
                    <div className="font-semibold">{car.brand} {car.model}</div>
                    <div className="text-slate-300">{car.segment} · {car.transmission === 'Automatic' ? 'Otomatik' : 'Manuel'} · {fuelLabel(car.fuel)}</div>
                    <div className="text-slate-400 text-xs mt-1">Min. yaş: {car.minAge}+ · Sürücü: {driverAge}+</div>
                  </div>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <div className="text-slate-300">{pickup.date} · {pickup.time}</div>
                  </div>
                  <div>
                    <div className="text-slate-400 text-xs">Bırakış</div>
                    <div className="font-medium">{drop.loc}</div>
                    <div className="text-slate-300">{drop.date} · {drop.time}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-white/10 p-4">
                ...
                <div className="mt-2 border-t border-white/10 pt-2 flex items-center justify-between">
                  ...
                  <div className="text-lg font-extrabold">{formatPrice(grandTotal(car, extras))}</div>
                </div>
                <div className="text-xs text-slate-400 mt-1">* Depozito araç grubuna göre provizyondur; toplam fiyata dahil değildir.</div>
              </div>
              ...
              <div className="rounded-xl border border-white/10 p-4">
                ...
                <ul className="mt-2 text-sm text-slate-300 space-y-1">
                  ...
                  {!Object.entries(EXTRAS_PRICING).some(([k]) => (k === 'childSeat' ? extras.childSeat > 0 : (extras as any)[k])) && (
              ...
              >
                <div className="col-span-2">
                  ...
                  <input value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 w-full h-10 rounded-lg bg-slate-900/60 border border-white/10 px-3" />
                </div>
                <div>
                  <div className="text-slate-300">Telefon</div>
                  <input value={phone} onChange={(e) => setPhone(e.target.value)} required className="mt-1 w-full h-10 rounded-lg bg-slate-900/60 border border-white/10 px-3" />
                </div>
                <div>
                  <div className="text-slate-300">E-posta</div>
                  <input type="email" value={mail} onChange={(e) => setMail(e.target.value)} required className="mt-1 w-full h-10 rounded-lg bg-slate-900/60 border border-white/10 px-3" />
                </div>

                <label className="col-span-2 flex items-center gap-2 text-xs text-slate-300">
                  <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} />
                  <span>Rezervasyon koşullarını okudum, kabul ediyorum.</span>
                </label>
