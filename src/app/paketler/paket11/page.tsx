...
/* =========================
   Types
========================= */
type Board = 'Room Only' | 'Bed & Breakfast' | 'Half Board' | 'Full Board' | 'All Inclusive' | 'Ultra All Inclusive';
type District = 'Konyaaltı' | 'Lara' | 'Belek' | 'Side' | 'Kemer' | 'Kaleiçi';
type Tag = 'Denize Sıfır' | 'Özel Plaj' | 'Aquapark' | 'Spa' | 'Aile Dostu' | 'Yalnız Yetişkin';
...
function toDate(d: string, t: string) {
  ...
  const dt = new Date(y, (m || 1) - 1, dd || 1, hh || 0, mm || 0, 0, 0);
...
/* =========================
   Page
========================= */
export default function Paket15AntalyaHotel() {
  /* ---- Search ---- */
  ...
  const [maxPrice, setMaxPrice] = useState(() => Math.max(...HOTELS.map(h => h.pricePerNight)));
  const [sort, setSort] = useState<SortKey>('pop');
  const [error, setError] = useState('');

  const nightCount = useMemo(() => nightsBetween(checkin, checkout), [checkin, checkout]);
  ...
  const filtered = useMemo(() => {
    ...
    if (tag !== 'Tümü') arr = arr.filter(h => h.tags.includes(tag as Tag));
  ...
  function submitSearch() {
    ...
    if (!nightCount) return setError('Çıkış tarihi, giriş tarihinden sonra olmalı.');
    if (adults < rooms) return setError('Her oda için en az 1 yetişkin gerekir.');
    setError('');
...
>
  ...
  <div className="absolute inset-0 -z-10 bg-gradient-to-b from-slate-900/30 via-slate-950 to-slate-950" />
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 md:py-26 grid md:grid-cols-12 gap-10 items-center">
          <div className="md:col-span-7">
            ...
            <p className="mt-4 text-lg text-slate-300 max-w-2xl">
              Denize sıfır lüks resortlardan Kaleiçi butiklerine. Esnek iptal, şeffaf fiyatlar, güvenli rezervasyon.
            </p>
          ...
          <aside id="search" className="md:col-span-5">
            <div className="rounded-2xl border border-teal-300/30 bg-white/[0.06] backdrop-blur p-5 shadow-[0_0_100px_-35px_rgba(94,234,212,0.55)]">
              <div className="text-sm text-slate-200 font-medium">Tarih & Misafir</div>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  submitSearch();
              ...
              >
                ...
                <Stepper label="Yetişkin" value={adults} setValue={setAdults} min={1} max={10} />
                <Stepper label="Çocuk" value={children} setValue={setChildren} min={0} max={10} />
                <Stepper label="Oda" value={rooms} setValue={setRooms} min={1} max={5} />
                ...
                <div className="col-span-2">
                  ...
                  <select
                    ...
                    className="mt-1 w-full h-11 rounded-xl bg-slate-900/60 border border-white/10 px-3"
                  >
                    {['Tümü', 'Konyaaltı', 'Lara', 'Belek', 'Side', 'Kemer', 'Kaleiçi'].map(d => (
                ...
                <div className="col-span-2 text-xs text-slate-300 flex items-center gap-3">
                  <span>⏱</span>
                  {nightCount ? <span>{nightCount} gece · {pax} kişi</span> : <span>Süre hesaplanamadı</span>}
                </div>
                ...
                <p className="col-span-2 text-[11px] text-slate-400">* Demo arama; sonuçlar aşağıdaki gridde filtrelenir.</p>
              </form>
            </div>
          </aside>
        </div>
      ...
      {/* Filters + List */}
      <section id="hotels" className="mx-auto max-w-7xl px-4 py-14 border-t border-white/10">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-semibold">Oteller</h2>
            <p className="text-slate-400 mt-1">
              {filtered.length} sonuç · {nightCount ? `${nightCount} gece` : 'Tarih seçin'}
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {(['Tümü', 'Denize Sıfır', 'Özel Plaj', 'Aquapark', 'Spa', 'Aile Dostu', 'Yalnız Yetişkin'] as const).map(t => (
          ...
          <div className="ml-auto hidden md:flex items-center gap-3 text-sm">
            ...
            <select
              ...
              className="h-10 rounded-lg bg-slate-900/60 border border-white/10 px-3 text-sm"
            >
              {(['Tümü', 'Room Only', 'Bed & Breakfast', 'Half Board', 'Full Board', 'All Inclusive', 'Ultra All Inclusive'] as const).map(b => (
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-300 flex items-center gap-2">
                      ...
                      <span>·</span>
                      <span>{starsLabel(h.stars)}</span>
                      <span>·</span>
                      <span>{h.board}</span>
                    </div>
                    <div className="mt-1 text-xs text-slate-400 flex items-center gap-2">
                      <span>🏝️ {h.beachDistance === 0 ? 'Denize Sıfır' : `${h.beachDistance} m`}</span>
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xl font-extrabold">{formatPrice(h.pricePerNight)}</div>
                    <div className="mt-1 text-xs text-slate-400">
                      {nightCount ? `Toplam: ${formatPrice(totalPrice(h))}` : 'Tarih seçin'}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {h.tags.slice(0, 4).map(t => (
                    <span key={t} className="px-2 py-0.5 rounded-full text-[11px] bg-slate-900/60 border border-white/10">{t}</span>
                  ))}
                </div>

                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button
                    onClick={() => { setPicked(h); setOpen(true); }}
                    disabled={!nightCount}
                    className={'col-span-2 h-11 rounded-xl font-semibold ' + (!nightCount ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : 'bg-teal-300 text-slate-900 hover:bg-teal-200')}
                  >
      ...
      {/* Map placeholder */}
      <section id="map" className="mx-auto max-w-7xl px-4 pb-16 border-t border-white/10">
      ...
      {/* FAQ */}
      <section id="faq" className="mx-auto max-w-7xl px-4 pb-20 border-t border-white/10">
        <h3 className="text-xl font-semibold mb-3">Sık Sorulan Sorular</h3>
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <Faq q="Ücretsiz iptal var mı?" a="Birçok tesiste giriş tarihinden 3 gün öncesine kadar ücretsiz iptal bulunur. Her tesisin iptal politikası kartta belirtilir." />
          <Faq q="Fiyatlar kişi başı mı, oda başı mı?" a="Fiyatlar varsayılan olarak 2 yetişkin tek oda içindir. Arama formundaki misafir/oda sayısına göre güncellenir." />
          <Faq q="Transfer hizmeti sağlanıyor mu?" a="Seçili otellerde ek ücretle havalimanı transferi sağlanır. Rezervasyon sırasında talep edebilirsiniz." />
          <Faq q="Çocuk indirimleri var mı?" a="Aile dostu tesislerde çocuk-yaş aralığına göre indirim uygulanır. Tesis detayında belirtilir." />
        </div>
      </section>
  ...
  <div className="p-4">
            ...
            <div className="mt-4 space-y-2 text-sm">
              ...
              <Row k="Misafir" v={`${adults} yetişkin${children ? `, ${children} çocuk` : ''}`} />
              <Row k="Oda" v={`${rooms}`} />
            </div>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="text-xs text-slate-400 mt-1">* 2 yetişkin başlangıç fiyatı, vergiler dahil (demo).</p>
            </div>

            <button
              disabled={!nightCount}
              className={'mt-4 w-full h-12 rounded-xl font-semibold ' + (!nightCount ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : 'bg-teal-300 text-slate-900 hover:bg-teal-200')}
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-11 rounded-xl bg-slate-900/60 border border-white/10 px-3 flex items-center justify-between">
        <button onClick={() => setValue(Math.max(min, value - 1))} className="h-7 w-7 rounded-lg border border-white/10">−</button>
        <span className="w-6 text-center">{value}</span>
        <button onClick={() => setValue(Math.min(max, value + 1))} className="h-7 w-7 rounded-lg border border-white/10">+</button>
      </div>
    </div>
  );
}
function Stars({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      {half && <span>☆</span>}
      {Array.from({ length: 5 - full - (half ? 1 : 0) }).map((_, i) => <span key={'e'+i} className="text-slate-500">☆</span>)}
  ...
  return (
    <button
      ...
      className={
        ...
        (active ? 'border-teal-300 text-teal-200 bg-teal-300/10' : 'border-white/15 hover:bg-white/5 text-slate-200')
      }
    >
      {children}
    </button>
...
function Faq({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5">
      <button onClick={() => setOpen(v => !v)} className="w-full text-left px-4 py-3 font-medium flex items-center justify-between">
        <span>{q}</span>
